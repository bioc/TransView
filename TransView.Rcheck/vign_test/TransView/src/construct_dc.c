...
int set_filter(SEXP filterList, filter_t * ft){
	...
	ft->sequence_name = (char**)R_Calloc(flen,char*);
	ft->values = (int32_t **)R_Calloc(flen,int32_t*);
	ft->value_length = (int32_t *)R_Calloc(flen,int32_t);
	PROTECT(fnames = AS_CHARACTER(fnames));upc++;
	int j=0;
	for(;j<flen;j++){
		PROTECT(fvals=AS_INTEGER(VECTOR_ELT(filterList, j)));upc++;
		ft->sequence_name[j] = R_Calloc(strlen(CHAR(STRING_ELT(fnames, j))), char);
...
void destroy_filter(filter_t * ft){
	int j=0;
	for(;j<ft->seqn;j++){
		if(ft->sequence_name[j])R_Free(ft->sequence_name[j]);
	}
	if(ft->sequence_name)R_Free(ft->sequence_name);
	if(ft->value_length)R_Free(ft->value_length);
	if(ft->values)R_Free(ft->values);
}
...
void write_density(global_densities_t * gd,user_arguments_t * user_args,samfile_t *bam_file,filter_t * ft){
	...
	proc_chrom=R_Calloc(bam_file->header->n_targets,uint32_t);
	...
	while(bresults.file_status>0 && chrom_tid<bam_file->header->n_targets){//loop through chromosomes and fetch densities
		...
		if(cs.min_indexspace<50)cs.min_indexspace=1000;//leave some space for testing
			...
			Rprintf("TID+1 %d <= TE %d\n",chrom_tid+1,bam_file->header->n_targets);
			Rprintf("Index Preallocated for %s: %d Scores Preallocated: %d | TID: %d CC: %d\n",bam_file->header->target_name[chrom_tid],cs.min_indexspace,cs.max_pos,chrom_tid,cc);
		#endif
		cptr = R_Calloc(cs.max_pos+1,usersize);
		lstart_ind=R_Calloc(cs.min_indexspace,uint32_t);
		lend_ind=R_Calloc(cs.min_indexspace,uint32_t);
		...
		for(scount_ind=0;scount_ind<cs.min_indexspace;scount_ind++){//go through the index
			...
			#if verbose==2
				Rprintf("FINALLY VALUE2@ %d -> %d || POS: %d\n",scount,scoresp[scount-1],bpos);
			#endif
			}
	...
	if(user_args->VERBOSE>0 && bresults.file_status>=0){
		if(cc<=bam_file->header->n_targets){
			...
			for(;x<bam_file->header->n_targets;x++){
				if(!proc_chrom[x]){
					Rprintf("%s ",bam_file->header->target_name[x]);
					++y;
					if(y%5==0)Rprintf("\n");
				}
			}
...
SEXP construct_dc(SEXP bamfilenameR, SEXP aRgvals, SEXP filterList) {
	...
	*statsp++=(double)gd.gsize;
    ...
    NO_READS_FOUND:
    time(&tstop);
	if(user_args.VERBOSE>0)Rprintf("About %.0f seconds passed. %llu reads processed \n", difftime(tstop, tstart),gd.total_reads);
